...
/// </summary>
public class BantamParser : ParserOriginal
{
    public BantamParser(LexerOriginal lexer) : base(lexer)
    {
    ...
    /// </summary>
    public void Postfix(TokenType token, int bindingPower) => Register(token, new PostfixOperatorParselet(bindingPower));

    /// <summary>
    /// Registers a prefix unary operator parselet for the given token and binding power.
    /// </summary>
    public void Prefix(TokenType token, int bindingPower) => Register(token, new PrefixOperatorParselet(bindingPower));

    /// <summary>
    ///  Registers a left-associative binary operator parselet for the given token and binding power.
    /// </summary>
    public void InfixLeft(TokenType token, int bindingPower) => Register(token, new BinaryOperatorParselet(bindingPower, false));

    /// <summary>
    /// Registers a right-associative binary operator parselet for the given token and binding power.
    /// </summary>
    public void InfixRight(TokenType token, int bindingPower) => Register(token, new BinaryOperatorParselet(bindingPower, true));
}
