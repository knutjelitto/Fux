...
{
    public ExprParser(Parser parser) => Parser = parser;
    ...
    public A.Expr Expression(Cursor cursor) => InfixExpr(cursor);

    private A.Expr InfixExpr(Cursor cursor) => cursor.Scope(cursor =>
                                                        {
    ...
    private A.Expr Sequence(Cursor cursor, bool always) => cursor.Scope(cursor =>
                                                                    {
                                                                        ...
                                                                        return !always && expressions.Count == 1 ? expressions[0] : new A.Expr.Application(expressions);
                                                                    });

    private A.Expr PrefixExpr(Cursor cursor) => cursor.Scope(cursor =>
                                                         {
                                                             A.Expr app;
    ...
    private A.Expr Compound(Cursor cursor) => cursor.Scope(cursor =>
                                                       {
    ...
    private A.Expr IfExpr(Cursor cursor) => cursor.Scope(cursor =>
                                                     {
    ...
    private A.Expr LetExpr(Cursor cursor) => cursor.Scope(cursor =>
                                                      {
    ...
    private A.Expr CaseExpr(Cursor cursor) => cursor.Scope(cursor =>
                                                       {
                                                           if (cursor.Line == 578)
                                                           {
                                                               Assert(true);
                                                           }
                                                           _ = cursor.Swallow(Lex.KwCase);
                                                           var expression = Expression(cursor);
                                                           _ = cursor.Swallow(Lex.KwOf);
    ...
    private A.Expr Atom(Cursor cursor) => cursor.Scope(cursor =>
                                                   {
                                                       Assert(cursor.StartsAtomic);
                                                       ...
                                                       A.Expr.Select SelectExpr(Cursor cursor, A.Expr atom) => cursor.Scope(cursor =>
                                                                                                                                {
    ...
    private A.Expr TupleLiteral(Cursor cursor) => cursor.Scope(cursor =>
                                                           {
                                                               ...
                                                               return expressions.Count == 0 ? new A.Expr.Unit() : expressions.Count == 1 ? expressions[0] : new A.Expr.Tuple(expressions);
                                                           });

    private A.Expr RecordLiteral(Cursor cursor) => cursor.Scope<A.Expr>(cursor =>
                                                            {
                                                                var left = cursor.Swallow(Lex.LeftCurlyBracket);

                                                                if (cursor.Is(Lex.RightCurlyBracket))
                                                                {
                                                                ...
                                                                var baseName = Parser.SingleIdentifier(cursor).SingleLower();
    ...
    private A.Expr.List ListLiteral(Cursor cursor) => cursor.Scope(cursor =>
                                                               {
    ...
    private A.Expr DotSelector(Cursor cursor) => cursor.Scope(cursor =>
                                                          {
                                                              _ = cursor.Swallow(Lex.Dot);
    ...
    public A.Expr.Literal.Char CharLiteral(Cursor cursor) => cursor.Scope(cursor =>
                                                                      {
    ...
    public A.Expr.Literal.String StringLiteral(Cursor cursor) => cursor.Scope(cursor =>
                                                                          {
    ...
    public A.Expr.Literal.LongString LongStringLiteral(Cursor cursor) => cursor.Scope(cursor =>
                                                                                  {
    ...
    public A.Expr.Literal.Integer IntegerLiteral(Cursor cursor) => cursor.Scope(cursor =>
                                                                            {
    ...
    public A.Expr.Literal.Float FloatLiteral(Cursor cursor) => cursor.Scope(cursor =>
                                                                        {
    ...
    public A.Expr.Case Case(Cursor cursor) => cursor.Scope(cursor =>
                                                       {
                                                           var pattern = Patt.Pattern(cursor);

                                                           _ = cursor.Swallow(Lex.Arrow);
    ...
    public A.Expr.Lambda LambdaLiteral(Cursor cursor) => cursor.Scope(cursor =>
                                                                  {
                                                                      _ = cursor.Swallow(Lex.Lambda);
