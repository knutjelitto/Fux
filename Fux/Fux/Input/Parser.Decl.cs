...
{
    public DeclParser(Parser parser) => Parser = parser;
    ...
    public PatternParser Ptrn => Parser.Patt;

    public A.Decl.Header ModuleHeader(Cursor cursor) => cursor.Scope(cursor =>
                                                                 {
                                                                     var effect = false;
                                                                     var port = false;
    ...
    public A.Decl.Import ImportDecl(Cursor cursor) => cursor.Scope(cursor =>
                                                               {
    ...
    private A.Exposing Exposing(Cursor cursor) => cursor.Scope<A.Exposing>(cursor =>
                                                           {
                                                               _ = cursor.Swallow(Lex.KwExposing);

                                                               if (cursor.IsWeak(Lex.Weak.ExposeAll))
                                                               {
                                                                   _ = cursor.Swallow(Lex.OperatorId);
    ...
    public A.Decl.Infix InfixDecl(Cursor cursor) => cursor.Scope(cursor =>
                                                             {
                                                                 _ = cursor.Swallow(Lex.KwInfix);
    ...
    public A.NamedDecl CustomOrAliasDecl(Cursor cursor) => cursor.Scope<A.NamedDecl>(cursor =>
                                                                    {
    ...
    public A.Decl.TypeParameter TypeParameter(Cursor cursor) => cursor.Scope(cursor => new A.Decl.TypeParameter(Module, Parser.SingleIdentifier(cursor).SingleLower()));

    public A.Decl VarDeclOrTypeAnnotationInLet(Cursor cursor) => cursor.Scope(cursor => cursor.StartsTypeAnnotation ? TypeAnnotation(cursor) : VarDecl(cursor, inLet: true));

    public A.Decl VarDecl(Cursor cursor, bool inLet) => cursor.Scope<A.Decl>(cursor =>
                                                                 {
                                                                     var pattern = Ptrn.Pattern(cursor);
                                                                     ...
                                                                     if (pattern is A.Pattern.Signature sign)
                                                                     {
                                                                         ...
                                                                         var parameters = new A.Parameters(prms.Select(pattern => new A.Decl.Parameter(Module, pattern)));

                                                                         return new A.Decl.Var(Module, name, parameters, expression);
                                                                     }
                                                                     else if (pattern is A.Pattern.LowerId lower)
                                                                     {
                                                                         var name = lower.Identifier.SingleLowerOrOp();

                                                                         if (inLet)
                                                                         {
                                                                             return new A.Decl.LetAssign(Module, pattern, expression);
                                                                         }
                                                                         else
                                                                         {
    ...
    public A.Decl.TypeAnnotation TypeAnnotation(Cursor cursor) => cursor.Scope(cursor =>
                                                                           {
    ...
    private A.Decl.Ctor Ctor(A.Decl.Custom custom, Cursor cursor) => cursor.Scope(cursor =>
                                                                              {
                                                                                  Assert(cursor.Is(Lex.UpperId));
    ...
    public A.Decl.ClassInstance Instance(Cursor cursor) => cursor.Scope(cursor =>
                                                                    {
                                                                        _ = cursor.Swallow(Lex.KwInstance);
                                                                        var className = Parser.Identifier(cursor).MultiUpper();
                                                                        var type = Type.Type(cursor);
                                                                        _ = cursor.Swallow(Lex.KwWhere);

                                                                        var vars = new List<A.Decl.Var>();
                                                                        do
                                                                        {
                                                                            var decl = (A.Decl.Var)VarDecl(cursor.Subcursor(), false);
                                                                            vars.Add(decl);
                                                                        }
                                                                        while (cursor.IsNot(Lex.EOF));

                                                                        return new A.Decl.ClassInstance(Module, className, type, vars);
                                                                    });

    public A.Decl.TypeClass TypeClass(Cursor cursor) => cursor.Scope(cursor =>
                                                                 {
                                                                     _ = cursor.Swallow(Lex.KwClass);
                                                                     ...
                                                                     if (cursor.SwallowIf(Lex.KwWhere))
                                                                     {
                                                                         do
                                                                         {
                                                                             var member = TypeAnnotation(cursor.Subcursor());
