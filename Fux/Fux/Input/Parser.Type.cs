...
{
    public TypeParser(Parser parser) => Parser = parser;

    public Parser Parser { get; }

    public A.Type Type(Cursor cursor) => cursor.Scope(cursor =>
                                                  {
    ...
    public A.Type TypeArgument(Cursor cursor) => cursor.Scope(cursor =>
                                                          {
    ...
    private A.Type BaseType(Cursor cursor) => cursor.Scope(cursor =>
                                                       {
                                                           if (cursor.Is(Lex.LowerId))
                                                           {
                                                               return LowerType(cursor);
    ...
    private A.Type.Custom UpperType(Cursor cursor, bool multi) => cursor.Scope(cursor =>
                                                                           {
    ...
    private A.Type LowerType(Cursor cursor) => cursor.Scope<A.Type>(cursor =>
                                                        {
                                                            var name = Parser.Identifier(cursor).SingleLower();

                                                            return name.ToString() == "number"
                                                                ? new A.Type.NumberClass(name)
                                                                : name.ToString() == "appendable"
                                                                ? new A.Type.AppendableClass(name)
                                                                : name.ToString() == "comparable" ? new A.Type.ComparableClass(name) : new A.Type.Parameter(name);
                                                        });

    private A.Type RoundType(Cursor cursor) => cursor.Scope(cursor =>
                                                        {
    ...
    private A.Type RecordType(Cursor cursor) => cursor.Scope(cursor =>
                                                         {
                                                             ...
                                                             if (cursor.IsNot(Lex.RightCurlyBracket))
                                                             {
