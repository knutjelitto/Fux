...
public sealed class Cursor
{
    ...
    public Token Swallow(Lex lexKind, [CallerMemberName] string? member = null) => this.Is(lexKind) ? Advance() : throw Error.Unexpected(lexKind, this.At(), member);

    public Token SwallowOp(string op, [CallerMemberName] string? member = null) => this.Is(Lex.Operator) && this.IsWeak(op) ? Advance() : throw Error.Unexpected(Lex.Operator, this.At(), member);
