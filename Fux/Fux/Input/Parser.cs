...
{
    public Parser(Module module, ErrorBag errors, Liner liner)
    {
        Module = module;
        Errors = errors;
        Liner = liner;
    ...
    public Source Source => Liner.Source;
    ...
    public A.Decl Outer(Cursor cursor) => cursor.Scope(cursor =>
                                                   {
                                                       A.Decl? outer = null;
                                                       ...
                                                       {
                                                           outer = cursor.Is(Lex.EOF)
                                                               ? new A.Eof(Module, cursor.Advance())
                                                               : cursor.Is(Lex.KwClass)
                                                                                                                          ? Decl.TypeClass(cursor)
                                                                                                                          : cursor.Is(Lex.KwInstance)
                                                                                                                                                                                     ? Decl.Instance(cursor)
                                                                                                                                                                                     : cursor.StartsTypeAnnotation ? Decl.TypeAnnotation(cursor) : Decl.VarDecl(cursor, inLet: false);
                                                       }

                                                       return cursor.More() ? throw Errors.Parser.Internal(cursor.At(), $"can not continue at token '{cursor.At()}'") : outer;
                                                   });

    public A.Identifier Identifier(Cursor cursor) => cursor.Scope(cursor =>
                                                              {
    ...
    public A.Identifier SingleIdentifier(Cursor cursor) => cursor.Scope(cursor =>
                                                                    {
