...
public sealed class ModuleScope : Scope
{
    ...
    private readonly Dictionary<A.Identifier, A.Decl.TypeClass> classes = new();
    private readonly List<A.Decl.ClassInstance> instances = new();

    public Module Module { get; }

    public ModuleScope(Module module) => Module = module;
    ...
    public void AddTypeClass(A.Decl.TypeClass decl)
    {
        var name = decl.Name.SingleUpper();

        Assert(!classes.ContainsKey(name));

        classes.Add(name, decl);
    }

    public void AddClassInstance(A.Decl.ClassInstance decl) => instances.Add(decl);

    public bool ImportAddImport(A.Decl.Import import) => imports.TryAdd(import.Name.MultiUpper(), import);

    public bool ImportAddAlias(A.Decl.Alias decl) => aliases.TryAdd(decl.Name.SingleUpper(), decl);

    public bool ImportAddType(A.Decl.Custom decl) => types.TryAdd(decl.Name.SingleUpper(), decl);

    public bool ImportAddInfix(A.Decl.Infix decl) => infixesIndex.TryAdd(decl.Name.SingleOp(), decl);

    public bool ImportAddConstructor(A.Decl.Ctor constructor) => constructors.TryAdd(constructor.Name.SingleUpper(), constructor);

    public bool ImportAddModule(A.Identifier name, Module module) => modules.TryAdd(name, module);

    public bool LookupImport(A.Identifier identifier, [MaybeNullWhen(false)] out A.Decl.Import import) => imports.TryGetValue(identifier.MultiUpper(), out import);
    ...
    public bool LookupInfix(A.Identifier identifier, [MaybeNullWhen(false)] out A.Decl.Infix infix) => infixesIndex.TryGetValue(identifier.SingleOp(), out infix);

    public bool LookupType(A.Identifier identifier, [MaybeNullWhen(false)] out A.Decl.Custom type) => types.TryGetValue(identifier.SingleUpper(), out type);

    public bool LookupAlias(A.Identifier identifier, [MaybeNullWhen(false)] out A.Decl.Alias alias) => aliases.TryGetValue(identifier.SingleUpper(), out alias);

    public bool LookupConstructor(A.Identifier identifier, [MaybeNullWhen(false)] out A.Decl.Ctor constructor) => constructors.TryGetValue(identifier.SingleUpper(), out constructor);

    public bool LookupNative(A.Identifier identifier, [MaybeNullWhen(false)] out A.Decl.Native native) => natives.TryGetValue(identifier.SingleLower(), out native);

    public bool LookupModule(A.Identifier identifier, [MaybeNullWhen(false)] out Module module) => modules.TryGetValue(identifier.MultiUpper(), out module);
