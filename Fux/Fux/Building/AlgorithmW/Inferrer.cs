...
public sealed class Inferrer
{
    ...
    public Environment GetEmptyEnvironment() => Environment.Initial(new TypeVarGenerator());

    private (Substitution, TType) InferType<TType>(Expr expression, Environment env)
        where TType : Type => ((Substitution, TType))InferType(expression, env);
    ...
    private (Substitution, Type) InferType(Expr expression, Environment env)
    {
        ...
        switch (expression)
        {
            ...
            case Expr.Variable({ } term):
                {
                    return env.TryGet(term) is Polytype polytype
                        ? (Substitution.Empty(), InstantiateType(polytype, env))
    ...
    /// </summary>
    private Environment ApplySubstitution(Environment env, Substitution substitution) => Environment.From(env, env.Enumerate().Select(vp => (vp.var, ApplySubstitution(vp.polytype, substitution))));
    ...
    private Polytype GeneralizePolytype(Environment env, Type type) => new(type, GetFreeTypeVariables(type).Except(GetFreeTypeVariables(env)).ToList());
            ...
            case (Type.Variable({ } v1) t1, Type.Variable({ } v2) t2):
                {
                    ...
                    return v1.ID > v2.ID ? BindVariable(v1, t2) : BindVariable(v2, t1);
                }
    ...
    /// </summary>
    private HashSet<TypeVariable> GetFreeTypeVariables(Environment typeEnv) => GetFreeTypeVariables(typeEnv.Polytypes);
