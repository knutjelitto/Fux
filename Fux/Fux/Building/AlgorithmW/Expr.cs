...
public abstract record Expr
{
    ...
    public abstract record Literal(Type Type) : Expr
    {
        ...
        {
            public override string ToString() => Math.Truncate(Value) == Value ? $"{Value}.0" : Value.ToString();
        }
    ...
    {
        public new sealed record Application(IReadOnlyList<Expr> Exprs) : Sugar
        {
            public override string ToString() => Pretty.ExprStr(this);
        }

        public new sealed record Lambda(IReadOnlyList<TermVariable> Terms, Expr Expr) : Sugar
        {
            public override string ToString() => Pretty.ExprStr(this);
        }

        public new sealed record Let(IReadOnlyList<(TermVariable term, Expr value)> Lets, Expr Expr) : Sugar
        {
            public override string ToString() => Pretty.ExprStr(this);
        }
    }
