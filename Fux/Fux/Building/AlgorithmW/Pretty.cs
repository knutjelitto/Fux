...
public sealed class Pretty
{
    ...
    public static string ExprStr(Expr expr) => Instance.Str(expr);

    public Pretty(Writer writer) => this.writer = writer;

    public Writer writer { get; }

    public void PP(Expr top) => Print(top);
    ...
    private string Str(Expr expr, int prio = 1)
    {
        switch (expr)
        {
            ...
            case Expr.Field field:
                {
                    return field.Value == null ? $"{field.Name}" : $"{field.Name} = {Str(field.Value)}";
                }
        ...
        string Clamp(string str) => prio > 0 ? $"({str})" : str;
    }
    ...
    private void WriteLine(string? text = null) => writer.WriteLine(text ?? string.Empty);

    private void Write(string? text = null) => writer.Write(text ?? string.Empty);

    private void EndLine() => writer.EndLine();
}
