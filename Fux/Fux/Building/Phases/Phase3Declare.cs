        ...
        private void Declare(Module Module, A.Decl declaration)
        {
            switch (declaration)
            {
                case A.Decl.Import import:
                    {
                        ...
                        _ = Module.Scope.ImportAddImport(import);
                ...
                case A.Decl.Var var:
                    {
                        ...
                        foreach (var parameter in var.Parameters)
                        {
                            if (parameter.Pattern is A.Pattern pattern)
                            {
                                ...
                                {
                                    var.Scope.Add(new A.Decl.Parameter(Module, identifier));
                                }
                            }
                            else
                            {
                                Assert(parameter.Pattern is not null);
                                throw new InvalidOperationException();
                            }
                        }
                ...
                case A.Decl.TypeClass typeClass:
                    {
                        Collector.Add(typeClass);
                        Module.Scope.AddTypeClass(typeClass);

                        break;
                    }

                case A.Decl.ClassInstance classInstance:
                    {
                        Collector.Add(classInstance);
                        Module.Scope.AddClassInstance(classInstance);
                ...
                case A.Expr.Case matchCase:
                    {
                        ...
                        foreach (var parameter in matchCase.Pattern.ExtractMatchNames(Module))
                        {
                ...
                case A.Expr.Lambda lambda:
                    {
                        ...
                        foreach (var parameter in lambda.Parameters.SelectMany(p => p.ExtractMatchNames(Module)))
                        {
                    ...
                    case A.Decl.Var var:
                        {
                            ...
                            foreach (var parameter in var.Parameters)
                            {
                                if (parameter.Pattern is A.Pattern pattern)
                                {
                                    foreach (var identifier in pattern.ExtractNamedParameters(Module))
                                    {
                    ...
                    case A.Decl.LetAssign assign:
                        {
                            ...
                            foreach (var identifier in assign.Pattern.ExtractNamedParameters(Module))
                            {
