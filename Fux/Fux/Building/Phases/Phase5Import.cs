...
public sealed class Phase5Import : Phase
{
    ...
    private void Import(Module module, Decl.Import import)
    {
        ...
        {
            _ = module.Scope.ImportAddModule(importName, import.InModule);
        }
    }

    private void Expose(Module module, Decl.Import import, Exposed exposed, bool inclusive)
    {
        ...
        switch (exposed)
        {
            case Exposed.Type exposedType:
                ...
                {
                    _ = module.Scope.ImportAddType(type);

                    if (inclusive)
                    {
                        foreach (var ctor in type.Ctors)
                        {
                            _ = module.Scope.ImportAddConstructor(ctor);
                        }
                    }
                }
                else if (import.InModule.Scope.LookupAlias(exposedType.Name, out var alias))
                {
                    _ = module.Scope.ImportAddAlias(alias);
                }
                else
                {
                    Assert(false);
