...
public class TypeNode
{
    ...
    public bool OccursInType(TypeNode other) => this == other || other.Name != null && other.ArgumentTypes != null && OccursIn(other.ArgumentTypes);

    public bool OccursIn(List<TypeNode> Types) => Types.Any(a => OccursInType(a));
...
public class IntNode : INode
{
    ...
    public TypeNode FindType(Dictionary<string, TypeNode> Environment, List<TypeNode> NonGeneric) => new() { Name = "Int" };
}

public class IdentityNode : INode
{
    public IdentityNode(string value) => Value = value;

    public string Value { get; set; }

    public TypeNode FindType(Dictionary<string, TypeNode> Environment, List<TypeNode> NonGeneric) => !Environment.ContainsKey(Value)
            ? throw new Exception(string.Format("Invalid identifier: {0}", Value))
            : Environment[Value].FreshCopy(new IdentityDictionary<TypeNode, TypeNode>(), NonGeneric);
}
