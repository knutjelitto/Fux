...
public class Scope
{
    ...
    public void AddVar(A.Decl.Var decl)
    {
        ...
        {
            decl.Type = hint.Type;
            _ = annotations.Remove(name);
        }
    }

    public bool ImportAddVar(A.Decl.Var decl) => vars.TryAdd(decl.Name.SingleLower(), decl);

    public bool LookupVar(A.Identifier identifier, [MaybeNullWhen(false)] out A.Decl.Var var) => vars.TryGetValue(identifier.SingleLowerOrOp(), out var);
