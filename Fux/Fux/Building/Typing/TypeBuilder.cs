                ...
                case A.Type.Custom custom:
                    {
                        ...
                        var arguments = custom.Arguments.Select(t => Resolve(t)).ToArray();
                        ...
                        switch (name)
                        {
                            ...
                            case $"Char.{Lex.Primitive.Char}":
                                return W.Type.Char.Instance;
                            case $"List.{Lex.Primitive.List}":
                                {
                                    Assert(arguments.Length == 1);
                                    return new W.Type.List(arguments[0]);
                                }
                        }

                        Assert(name != "Bool.Bool");
                        Assert(name != "List.List");
                        return new W.Type.Custom(name, arguments);
                    }
