...
{
    private const string GenTuple2Prefix = "_tuple2";
    private const string GenVarPrefix = "_var";
    private const string GenMatchPrefix = "_match";
    private const string GenParamPrefix = "_param";
    ...
    private W.Expr Build(A.Expr expr)
    {
        ...
        switch (expr)
        {
            ...
            case A.Expr.Literal.Float literal:
                {
                    return new W.Expr.Literal.Float(literal.Value);
                }
        ...
        W.Expr.List Cons(A.Expr.List list, int index) => index == list.Count ? new W.Expr.Empty() : new W.Expr.Cons(Build(list[index]), Cons(list, index + 1));
            ...
            case A.Pattern.Tuple2(var pattern1, var pattern2):
                {
                    ...
                    [DebuggerStepThrough]
                    static W.Expr Select(W.Expr.Variable variable, int index) => new W.Expr.GetValue(
                            variable,
            ...
            case A.Pattern.Cons cons:
                {
                    ...
                    W.Expr.GetValue select(W.Expr expr, int index) => new(
                            expr,
    ...
    private W.Expr BuildLetExpr(A.Expr.Let letExpr)
    {
        ...
        var inExpr = Build(letExpr.InExpression);

        for (var i = letExpr.LetDecls.Count - 1; i >= 0; i--)
        {
            var let = letExpr.LetDecls[i];

            if (let is A.Decl.TypeAnnotation)
            {
                continue;
            }
    ...
    private W.Expr Decompose(A.Pattern pattern, W.Expr expr)
    {
        switch (pattern)
        {
            case A.Pattern.LowerId lower:
    ...
    private W.Expr Let(A.Pattern pattern, W.Expr get, W.Expr expr)
    {
        switch (pattern)
        {
            case A.Pattern.LowerId lower:
    ...
    private static Exception NotImplemented(A.Node node) => new NotImplementedException($"missing implementation: '{node.GetType().FullName} - {node}'");

    public A.Identifier GenWildcard() => idGenerator.For("_");

    public W.Expr.Variable GenVariable(string prefix) => new(idGenerator.For(prefix));
}
