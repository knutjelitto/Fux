...
public sealed class Collector
{
    ...
    public void Add(A.Decl declaration) => Declarations.Add(declaration);
    ...
    public void Write()
    {
        ...
        void WriteVarPatterns(string name) => WriteAll(name, StringsFrom(VarPattern));

        void WriteMatchPatterns(string name) => WriteAll(name, StringsFrom(MatchPattern));

        void WriteLetPatterns(string name) => WriteAll(name, StringsFrom(LetPattern));

        IEnumerable<string> StringsFrom(IEnumerable<A.Node> exprs) => exprs
                .Select(p => p.ToString()!)
        ...
        void writeStr(Writer writer, A.Decl expr) => writer.WriteLine($"{expr}");
        ...
        void WriteCompact(string name, IEnumerable<A.Decl> declarations, Action<Writer, A.Decl> write)
        {
            using (var writer = name.Writer())
            {
                ...
                foreach (var declaration in declarations)
                {
                    ...
                    writer.Indent(() => write(writer, declaration));
                }
            }
        }
    }
